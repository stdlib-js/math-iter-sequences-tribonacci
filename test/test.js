...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof iterTribonacciSeq, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object which generates a Tribonacci sequence', function test( t ) {
	var expected;
	var actual;
	var it;
	var i;

	it = iterTribonacciSeq();
	t.equal( it.next.length, 0, 'has zero arity' );

	for ( i = 0; i < 64; i++ ) {
		actual = it.next();
		expected = tribonacci( i );
...
tape( 'if an environment supports `Symbol.iterator`, the returned iterator is iterable', function test( t ) {
	var iterTribonacciSeq;
	var it1;
	var it2;
	var i;

	iterTribonacciSeq = proxyquire( './../lib/main.js', {
		'@stdlib/symbol-iterator': '__ITERATOR_SYMBOL__'
	});

	it1 = iterTribonacciSeq();
